...
zbottom = abs(fjord_run.p.zgl); % uses fjord grounding line as the lower integration boundary
...
for i_time=1:length_run
    ...
    if kbottom==1
        h_int(1,i_time) = abs(zbottom); % if GL is in the first box, we take the GL depth as 'h_int'
    else
        for k=1:kbottom
            if k==kbottom
