...
tau_space = [5, 10, 30, 90, 365]; % relaxation time - trelax = [10, 3650] days
...
for i_tau=1:length(tau_space)
    name_exp = sprintf('%s_tau%dd_fixed',fjord_keys{id},tau_space(i_tau));    
    ...
    fjord_run.p.trelax = tau_space(i_tau)*86400;
    tic
...
% Control run:
fjord_run = prepare_boxmodel_input(datasets,fjords_processed(fjord_ids(id)),fjord_ids(id)); % arranges into the boxmodel input structure    
fjord_run.p.trelax = NaN;
fjord_run.s = boxmodel_v4(fjord_run.p,fjord_run.f,fjord_run.a,fjord_run.t); % runs and gets the results    
fjord_run.o = postprocess_boxmodel(fjord_run);
fjord_run.m.name = sprintf('%s_tauoff',fjord_keys{id});
save([outs_path,name_exp,'.mat'],'fjord_run','-v7.3') % v7.3 allows files > 2GB
hf=plot_ts_at_depth(fjord_run,[5,200,500],'nearest');
print([figs_path,'rlx_test/',fjord_run.m.name],'-dpng','-r300')

name_set = sprintf('%s_tau*',fjord_keys{id});
plot_subset_runs([outs_path,name_set,'.mat'],'Effect of relaxation','Paired')

for i_tau=1:length(tau_space)
    name_exp = sprintf('%s_tau%dd',fjord_keys{id},tau_space(i_tau));    
    fjord_run = load([outs_path,name_exp,'.mat']).fjord_run;
    hf=plot_ts_at_depth(fjord_run,[5,200,500],'nearest');
    print([figs_path,'rlx_test/',name_exp],'-dpng','-r300')
    close gcf;
end
