...
tau_space = [10, 30, 90, 365]; % relaxation time - trelax = [10, 3650] days

% test K0 and wmax?
n_dt=length(dt_space); n_C0=length(C0_space); n_P0=length(C0_space); n_wm=length(wm_space); n_K0=length(K0_space);

%% Single-parametre run
for i_tau=1:length(tau_space)
    dt_run = dt_space(3);    
    name_exp = sprintf('%s_C%1.e_P%d_dt%0.2f_tau%dd_havg',fjord_keys{id},C0_space(3),P0_space(2),dt_run,tau_space(i_tau));    
