...
%% Plotting Potential density
figure('Name','Density profiles','position',[40 40 1000 400])
for i_reg=1:length(regions_lbl)
    subplot(2,4,i_reg); hold on; box on; grid on;
    salt_bnds = prctile(salt_forcing(:,:,i_reg),[5 95],1);
    temp_bnds = prctile(temp_forcing(:,:,i_reg),[5 95],1);
    ...
    sigma_median_profile = gsw_sigma0(salt_median_profile,temp_median_profile);
    sigma_lower_profile  = gsw_sigma0(salt_lower_profile,temp_lower_profile);
    sigma_upper_profile  = gsw_sigma0(salt_upper_profile,temp_upper_profile);

    y2 = [depths, fliplr(depths)];
    inBetween = [sigma_lower_profile, fliplr(sigma_upper_profile)];
    
    subplot(2,4,i_reg); hold on; box on; grid on;
    hp = fill(inBetween, y2, region_line_color(i_reg,:),'edgecolor','none','facealpha',0.2);
    plot(sigma_lower_profile,depths,'color',region_line_color(i_reg,:));
    plot(sigma_upper_profile,depths,'color',region_line_color(i_reg,:));
    plot(sigma_median_profile,depths,'color',region_line_color(i_reg,:));
    ylim([min(depths) 0])
    xlim([25.5 28])
    if i_reg==1 || i_reg==5, ylabel('Depth (m)'); end
    if i_reg>3, xlabel('Potential density \sigma (kg m^{-3})'); end
    set(gca,'fontsize',14)
    text(0.03,1.08,sprintf('(%s) %s',letters{i_reg},regions_lbl{i_reg}),'units','normalized','fontsize',14)
end
subplot(2,4,8); hold on; box on; grid on;
