function [file_out,tgt_days] = run_extra_ensemble_for_year(which_year,fjords_digitised,fjords_centreline,fjord_matrix,folder_ctd_casts,X,param_names,n_years,tgt_days,dt_in_h,dt_plume_h,plot_ensemble)
...
n_runs = 3;                                % we only have 3 runs per fjord: A0 disabled, Wp disabled, C0 very low
...
ensemble_extra = struct(ensemble_fields{:});
ensemble_extra(dims_ensemble) = struct("p",[],"t",[],"m",[],"s",[]);
fjords_crashed = {};
...
% run_counter=0;
tic
parfor i_fjord=1:length(fjord_model)
% for i_run=1:n_runs
for i_run=1:3
    ...
    for i_param=1:size(X,2)
        cur_fjord.p.(param_names{i_param}) = mean(X(ceil((size(X,1):size(X,1)+1)/2),i_param));
    end
    ...
    switch(i_run)
        case 1
            cur_fjord.p.M0 = 0; % turn off iceberg melting
            cur_fjord.p.A0 = 0;
        case 2
            cur_fjord.f.Qsg(cur_fjord.f.Qsg ~= 0) = 0.0; % turn off subglacial discharge
        case 3
            cur_fjord.p.C0 = 1e-2; % severely restrict fjord-shelf exchanges
    end
    ...
    % cur_fjord.p.plot_runtime=1; % for debuggging purposes if needed
        try
            ...
            ensemble_extra(i_fjord,i_run).p   = cur_fjord.p;
            ensemble_extra(i_fjord,i_run).t   = cur_fjord.t;
            ensemble_extra(i_fjord,i_run).m   = cur_fjord.m;
            ensemble_extra(i_fjord,i_run).s.t = cur_fjord.s.t;
            ensemble_extra(i_fjord,i_run).s.Qsg_max = max(cur_fjord.s.Qsg);
            ...
            ensemble_extra(i_fjord,i_run).s.Tfinal   = Tfinal;
            ensemble_extra(i_fjord,i_run).s.Sfinal   = Sfinal;
            ensemble_extra(i_fjord,i_run).s.QVsfinal = QVsfinal;
            ensemble_extra(i_fjord,i_run).s.QVpfinal = QVpfinal;
            ensemble_extra(i_fjord,i_run).s.QMpfinal = QMpfinal;
            ensemble_extra(i_fjord,i_run).s.Tmean    = T_mean;
            ensemble_extra(i_fjord,i_run).s.Smean    = S_mean;
            ensemble_extra(i_fjord,i_run).s.QMpmean  = QMp_mean;

            ensemble_extra(i_fjord,i_run).s.Tforc = mean(cur_fjord.s.Ts(:,(tgt_day-5:tgt_day+5)),2); 
            ensemble_extra(i_fjord,i_run).s.Sforc = mean(cur_fjord.s.Ss(:,(tgt_day-5:tgt_day+5)),2); 
            ...
            ensemble_extra(i_fjord,i_run).s.fw_profile_export    = fw_mean_export_profile;
            ensemble_extra(i_fjord,i_run).s.fw_profile_discharge = fw_mean_discharge_profile;
            ensemble_extra(i_fjord,i_run).s.fw_export            = fw_export;
            ensemble_extra(i_fjord,i_run).s.fw_export_t          = fw_export_t;
            ensemble_extra(i_fjord,i_run).s.z_max_export         = cur_fjord.s.z(i_max_export);
            ensemble_extra(i_fjord,i_run).s.z_max_export_t       = cur_fjord.s.z(i_max_export_t);
            ensemble_extra(i_fjord,i_run).s.Qsg                  = cur_fjord.s.Qsg;

            ensemble_extra(i_fjord,i_run).s.znb                  = cur_fjord.s.z(inb);
            ensemble_extra(i_fjord,i_run).s.znb_t                = cur_fjord.s.z(knb_t);
            ensemble_extra(i_fjord,i_run).s.znb(inb_mask==0)        = NaN;
            ensemble_extra(i_fjord,i_run).s.znb_t(knb_mask==0)      = NaN;
            ...
            ensemble_extra(i_fjord,i_run).s.z      = z_box;
            ensemble_extra(i_fjord,i_run).s.Tupper = mean(Tregular(range_upper,:),1,'omitnan'); % avg temp of the upper 50 m
            ensemble_extra(i_fjord,i_run).s.Tinter = mean(Tregular(range_inter,:),1,'omitnan'); % avg temp of the 50 - 250 m layer
            ensemble_extra(i_fjord,i_run).s.Tlower = mean(Tregular(range_lower,:),1,'omitnan'); % avg temp of the 250 - 500 m layer

            ensemble_extra(i_fjord,i_run).s.Supper = mean(Sregular(range_upper,:),1,'omitnan'); % avg temp of the upper 50 m
            ensemble_extra(i_fjord,i_run).s.Sinter = mean(Sregular(range_inter,:),1,'omitnan'); % avg temp of the 50 - 250 m layer
            ensemble_extra(i_fjord,i_run).s.Slower = mean(Sregular(range_lower,:),1,'omitnan'); % avg temp of the 250 - 500 m layer
...
toc
file_out = [outs_path,'rpm_GRL_fjords_extra_',num2str(which_year),'_dtp',num2str(dt_plume_h),'h_dtm',num2str(dt_in_h),'h.mat'];
save(file_out,'-v7.3','ensemble_extra','fjord_model');
